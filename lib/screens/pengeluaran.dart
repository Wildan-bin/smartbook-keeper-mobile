...
class _PengeluaranState extends State<Pengeluaran> {
  ...
  Future<void> _fetchTransactions() async {
    ...
    try {
      ...
      if (res.statusCode == 200) {
        ...
        final expenseOnly = all.where((t) {
          final type = t['type']?.toString().toLowerCase() ?? '';
          return type == 'expense';
        }).toList();

        debugPrint('Expense transaksi: ${expenseOnly.length}');
  ...
  Future<void> _submitTransaction() async {
    ...
    try {
      ...
      if (res.statusCode == 201) {
        ...
        _showSnackBar("Pengeluaran berhasil ditambahkan!", Colors.green);
        await _fetchTransactions();
        await _fetchBalances();
      } else {
        final data = json.decode(res.body);
        _showSnackBar(
          data['message'] ?? "Gagal menambah pengeluaran",
          Colors.red,
        );
      }
    } catch (e) {
  ...
  Future<void> _updateTransaction(String id) async {
    ...
    try {
      ...
      if (res.statusCode == 200) {
        _resetForm();
        Navigator.pop(context);
        _showSnackBar("Pengeluaran berhasil diperbarui!", Colors.green);
        await _fetchTransactions();
        await _fetchBalances();
      } else {
        final data = json.decode(res.body);
        _showSnackBar(
          data['message'] ?? "Gagal memperbarui pengeluaran",
          Colors.red,
        );
      }
    } catch (e) {
  ...
  /// Hapus pengeluaran dengan konfirmasi
  Future<void> _deleteTransaction(String id) async {
    ...
    try {
      ...
      if (res.statusCode == 200) {
        _showSnackBar("Pengeluaran berhasil dihapus!", Colors.green);
        await _fetchTransactions();
        await _fetchBalances();
      } else {
        _showSnackBar("Gagal menghapus pengeluaran", Colors.red);
      }
    } catch (e) {
      _showSnackBar("Kesalahan jaringan", Colors.red);
    }
  ...
  double _getTotalExpense() {
    debugPrint('=== DEBUG _getTotalExpense ===');
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      // ✅ GANTI: NestedScrollView TopBar dengan AppBar TopBar
      ...
      body: RefreshIndicator(
        ...
        child: CustomScrollView(
          slivers: [
            // 🔹 Header Summary Card
            SliverToBoxAdapter(
              child: _buildSummaryCard(totalExpense, transactionCount),
            ),

            // 🔹 Transaction List
            _transactions.isEmpty
                ? SliverFillRemaining(
                    child: EmptyState(message: "Belum ada data pengeluaran"),
                  )
                : SliverPadding(
  ...
  Widget _buildSummaryCard(double totalExpense, int count) {
    return Container(
      margin: const EdgeInsets.all(16),
        ...
        children: [
            ...
            children: [
              Column(
                ...
                children: [
                  ...
                  Text(
                    _formatCurrency(totalExpense.toInt()),
                    style: GoogleFonts.manrope(
                      color: Colors.white,
                      fontSize: 28,
          ...
          Row(
            ...
            children: [
              ...
              _buildStatItem(
                'Rata-rata',
                _formatCurrency(count > 0 ? totalExpense ~/ count : 0),
                Icons.calculate,
              ),
            ],
          ),
